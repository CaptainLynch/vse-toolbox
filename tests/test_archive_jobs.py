...
def test_archive_schema_version_and_tables(db: DatabaseManager) -> None:
    ...
    expected_tables = {
        ...
        "scheduled_archive_config_audit",
    }
    ...
    with db.get_connection() as conn:
        ...
        audit_cols = {row["name"] for row in conn.execute("PRAGMA table_info(scheduled_archive_config_audit)")}
        assert {
            "id",
            "job_id",
            "job_key",
        ...
        assert "idx_archive_config_audit_job" in audit_indices
