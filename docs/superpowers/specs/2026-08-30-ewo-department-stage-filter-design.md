...
审计约束：候选框背景必须引用已定义的主题不透明变量（如 `--surface-card`），并设置足够的层级和阴影；阶段标签必须在所有用户可见位置大写而请求值保持小写；待签署输入允许字符串、`ROLE:person` 行、角色到人员的对象/数组及常见分隔符，未知角色原样保留，空值为“无”；EWO 策略编辑器需对 `VPI-T2-D3` 提供三模式展示和“推荐：自动同步”文案，映射未配置时显示未启用状态，不得声称已同步；手工进度和快照摘要必须有互不混淆的来源标签。

第二轮数据契约细化：待签署只读取待签署/待审批等专用字段，字段优先级按现有 alias 顺序，绝不回退 `负责人`；规范化为有序的 `[{role, person}]`，同角色多人员保留多条，空白行丢弃，支持换行、分号、中文逗号和英文逗号分隔，重复的完全相同角色/人员只保留首次，最终 API 兼容输出为每条一行 `ROLE:person`。更新策略状态以既有 policy API 的 `enabled` 与 `mapping` 为准：`enabled !== true` 或 mapping 不完整显示“未启用”，只有二者满足既有校验才可显示已配置/可同步；三种模式展示不改变未配置时的同步行为。多选沿用既有 `departments`/`stages` 重复键、每类最多 20 项、单项最多 120 字符、去重和控制字符拒绝规则，未知科室返回空集。
